...
import { LayoutDashboard, LogOut, Bell, User, Eye, EyeOff } from "lucide-react";
    ...
    <div className="min-h-screen bg-background flex font-sans">
      ...
      <aside className="w-60 bg-white border-r border-border flex-col shrink-0 hidden md:flex fixed h-full z-20">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 text-sm rounded-md transition-colors ${isActive ? "bg-primary/10 text-primary font-medium border-l-2 border-primary" : "text-muted-foreground hover:text-foreground hover:bg-secondary/50 font-medium"}`}
              >
        ...
        <div className="p-4 mt-auto">
          <Button
            variant="ghost"
            className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-muted-foreground hover:text-foreground rounded-md transition-colors"
            onClick={() => {
              logout();
              router.replace("/login");
            }}
  ...
  const notifications = [
    {
      ...
      body: "Order #123 has been successfully funded. Status updated to Ready to Ship.",
    },
    {
      title: "Order Dispatched",
      body: "Order #123 has been marked as dispatched. Rider verification link generated.",
    },
    {
      title: "Payout Released",
      body: "₦15,000 has been transferred to your bank account.",
    },
    {
      title: "Payment Expired",
      body: "Order #123 payment window has expired.",
    },
    {
      title: "Underpayment Detected",
      body: "Buyer has paid ₦10,000 of ₦15,000. Awaiting balance payment.",
    },
      ...
      <SheetContent side="right" className="w-90 sm:w-100 p-0 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {notifications.map((n, i) => (
            <div
              key={i}
              className="p-4 border-b border-border hover:bg-slate-50 transition-colors"
            >
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="rounded-full w-9 h-9 bg-secondary"
        ...
        <div className="mt-4">
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-200 text-amber-800 p-3 rounded-md text-sm mt-4">
              Note: After updating your password, you will be logged out and
              redirected to the login page for security purposes.
            </div>
